...
export default function PublicDeliveryPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setData((prev) => prev ? { ...prev, confirmed: true, confirmed_at: new Date().toISOString(), confirmation_comment: comment.trim() } : null);
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-semibold text-stone-900">{data.project.project_code}</p>
                <p className="text-base font-medium text-stone-700 mt-1">{data.project.project_name}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-medium text-stone-900">{data.project.client_name}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-stone-900">
                  {format(new Date(data.project.start_date), 'dd MMM yyyy')}
                  {data.project.end_date && (
                    <>
                      {' → '}
                      {format(new Date(data.project.end_date), 'dd MMM yyyy')}
                    </>
                  )}
                </p>
              </div>
        ...
        <div className="rounded-xl border bg-white">
          <div className="border-b px-5 py-3">
            <h2 className="text-sm font-semibold">
              {data.confirmed ? 'Receipt Confirmed' : 'Confirm Receipt'}
            </h2>
          </div>
          <div className="p-5">
            {data.confirmed ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {data.confirmed_at && (
                      <p className="text-xs text-green-700 mt-1">
                        Confirmed on {format(new Date(data.confirmed_at), 'dd MMM yyyy, h:mm a')}
                      </p>
                    )}
                    {data.confirmation_comment && (
